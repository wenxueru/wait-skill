...
2. Use the current harness's native Todo or plan tool, when available, to track the wait. Keep credentials out of output. Nothing to prepare for the resume message: the service always generates `$wait resume {log_file}; event_id={event_id}` on completion.
3. Submit `src/waitctl.py start -- ... -- <waiting program>` with the client and current session. The default limit is one hour. Assess task and trigger reliability before extending it; consider [wait-loop](wait-loop/SKILL.md) for periodic health and progress checks during long waits.
...
5. On resume, read the log, validate the event ID, inspect program output and exit code, and re-check external state before deciding the next action. Complete Todo only after acceptance; program exit is not business success.
