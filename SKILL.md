...
Move repeated checks for one external object into `scripts/wait_for.py`, then end the model turn until its event resumes the session. Read [the watcher reference](docs/wait.md) and the active [client adapter](docs/clients.md) before starting a wait.
...
3. Start `scripts/wait_for.py` with `--client` and the current `--session`. For a long wait, use a process manager available in the environment; prefer `tmux` only after confirming it exists.
...
- Do not switch to `wait-loop` because work repeats or to `wait-goal` because work is long or complex. They are separate, explicitly user-invoked Loop and Goal mode implementations.
