...
class WaitDaemonTest(unittest.IsolatedAsyncioTestCase):
    async def test_goal_program_activation_and_wake(self) -> None:
        ...
        prepared = await goal(
            ...
            "deployment",
            "--event-note",
            "Recheck deployment and continue",
            "--log-file",
            str(log),
            "--lock-file",
            str(lock),
        ...
        submitted = self.daemon.submit(
            [
                ...
                sys.executable,
                "-c",
                "print('Deployment failed'); raise SystemExit(2)",
            ],
            str(self.root),
        )
        ...
        with lock.open("a") as handle, self.assertRaises(BlockingIOError):
    ...
    async def test_restart_does_not_repeat_an_unconfirmed_program(self) -> None:
        ...
        recovered = restored.watchers[submitted["watch_id"]]
        self.assertEqual(recovered["recovery_action"], "reconcile_result")
        self.assertIn("recovered_at", recovered)
        self.assertEqual(recovered["result"]["event"], "interrupted")

    async def test_restart_recovers_a_durable_result_from_querying_phase(self) -> None:
        with patch.object(self.daemon, "_schedule"):
            submitted = self.daemon.submit(self.watcher_argv("Ready"), str(self.root))
        record = self.daemon.watchers[submitted["watch_id"]]
        args, _ = waitd.wait_runtime.parse_job_args(record["argv"])
        durable = {
            "event": "exited",
        ...
        waitd.wait_runtime.persist_result(args.log_file, durable)
        record["phase"] = "querying"
        self.daemon.save()

        restored = waitd.WaitDaemon(self.root / "registry.json")
        await restored.restore()
        await restored.tasks[submitted["watch_id"]]
        result = restored.watchers[submitted["watch_id"]]["result"]
        self.assertEqual(result["event"], "exited")
        self.assertEqual(result["stdout"], "Ready\n")

    async def test_watcher_failure_records_exception_details(self) -> None:
    ...
    async def test_many_watchers_finish_with_a_valid_registry(self) -> None:
            ...
            submitted.append(
                self.daemon.submit(
                    [
                        ...
                        f"stress-{index}",
                        "--lock-file",
                        str(self.root / f"stress-{index}.lock"),
                        "--log-file",
                        str(self.root / f"stress-{index}.json"),
                        "--",
                        sys.executable,
                        "-c",
                        f"print({index})",
        ...
        await asyncio.gather(*(self.daemon.tasks[item["watch_id"]] for item in submitted))
        registry = json.loads((self.root / "registry.json").read_text())
        for item in submitted:
            record = registry["watchers"][item["watch_id"]]
    ...
    async def test_minimal_wait_generates_paths_and_resume_message(self) -> None:
        ...
        self.assertTrue(await asyncio.to_thread(Path(submitted["log_file"]).is_file))
        self.assertTrue(await asyncio.to_thread(Path(submitted["lock_file"]).is_file))
        record = self.daemon.watchers[submitted["watch_id"]]
    ...
    async def test_automatic_lock_rejects_duplicate_without_overwriting_log(self) -> None:
        argv = ["--label", "minimal", "--timeout", "10", "--", sys.executable, "-c", "import time; time.sleep(30)"]
    ...
    async def test_native_goal_wait_retains_lock_until_ack_or_delivery_deadline(self) -> None:
        for acknowledge in (False, True):
            with self.subTest(acknowledge=acknowledge):
                ...
                args.wake_ack_timeout = 0.001
                args.notification_timeout = 0.15
                args.activation_interval = 0.005
                ...
                with patch.object(waitd.wait_runtime, "goal_wait_is_current", return_value=True) as current:
                    ...
                    with args.lock_file.open("a") as lock, self.assertRaises(BlockingIOError):
                        fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
                    self.assertEqual(record["wake_ack_deadline_at"], available_at + 0.15)
                    if acknowledge:
    ...
    async def test_follow_timeout_does_not_cancel_watcher(self) -> None:
        ...
        try:
            with self.assertRaisesRegex(ValueError, "follow timed out"):
                await self.daemon.dispatch({"operation": "follow", "watch_id": submitted["watch_id"], "timeout": 0.02})
    ...
    async def test_cancelled_program_is_reaped_before_lock_release(self) -> None:
        ...
        argv[-1] = (
            f"import os,time; from pathlib import Path; Path({str(pid_file)!r}).write_text(str(os.getpid())); time.sleep(30)"
        )
        submitted = self.daemon.submit(argv, str(self.root))
        task = self.daemon.tasks[submitted["watch_id"]]
        for _ in range(100):
    ...
    async def test_program_output_is_bounded_and_timeout_is_reported(self) -> None:
        ...
        result = await waitd.run_program([sys.executable, "-c", "import time; time.sleep(30)"], 0.05, str(self.root))
    ...
    async def test_codex_preflight_failure_does_not_start_query(self) -> None:
        ...
        with (
            ...
            self.assertRaisesRegex(ValueError, "notification_unavailable"),
        ):
            self.daemon.submit(
                [
                    "--label",
    ...
    async def test_codex_delivery_preserves_stderr_and_stops_retrying(self) -> None:
        ...
        command = [
            sys.executable,
            "-c",
            "import sys; print('failed to connect to remote app server', file=sys.stderr); raise SystemExit(1)",
        ]
        ...
        for mode in ("wait", "wait-loop", "wait-goal"):
            with self.subTest(mode=mode):
                ...
                if mode == "wait":
                    self.assertEqual(
                        message,
                        f'$wait resume {args.log_file}; event_id={args.event_id}; event_note="Read the log and continue"',
                    )
                elif mode == "wait-loop":
                    self.assertEqual(
                        message,
    ...
    async def test_complete_registers_timer_and_delivers_without_manual_start(self) -> None:
        ...
        with patch.object(self.daemon, "_deliver", new_callable=AsyncMock, return_value=None) as deliver:
            response = await self.daemon.dispatch({
                "operation": "loop",
                "cwd": str(self.root),
                "argv": ["complete", "--state", str(path), "--summary", "done"],
            })
    ...
    async def test_activation_uses_persisted_absolute_deadline(self) -> None:
        ...
        outcome = await asyncio.wait_for(
            self.daemon._activate_goal_wait(args, asyncio.Event(), time.time() + 0.01),
            timeout=1,
        )
    ...
    async def test_uncertain_delivery_is_not_replayed(self) -> None:
        ...
        resumed, code = await self.daemon._watch(
            args,
                ...
                submitted = waitctl.request(
                    {
                        ...
                        "argv": [
                            ...
                            sys.executable,
                            "-c",
                            "import time; time.sleep(.1); print('Ready')",
                        ],
                    },
                    socket_path,
                )
                followed = waitctl.request(
                    {"operation": "follow", "watch_id": submitted["watch_id"], "timeout": 2}, socket_path
                )
    ...
    def test_goal_wait_program_round_trip(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.spawn_waitd(root) as (socket_path, _process):
                ...
                goal("init", "--state", str(state), "--objective", "verify", "--client", "claude", "--session", "e2e")
                ...
                followed = waitctl.request(
                    {"operation": "follow", "watch_id": watch_id, "timeout": 5},
                    socket_path,
                )
                self.assertEqual(followed["result"]["event"], "exited")
