...
2. 当前 harness 提供原生 Todo／计划工具时，用它记录等待进度。输出中不要包含凭据。唤醒消息无需准备：完成后服务总是生成 `$wait resume {log_file}; event_id={event_id}`。
3. 通过 `src/waitctl.py start -- ... -- <等待程序>` 提交，指定客户端和当前会话。默认上限一小时；延长前检查任务与条件是否可靠，长等待需要定期检查健康和进展时可用 [wait-loop](wait-loop/SKILL.zh-CN.md)。
4. 保留返回的 watch ID、日志和锁路径，用 `show` 和持有的锁确认已启动。按[客户端说明](docs/clients.zh-CN.md)接好投递方式，在 Todo 记录等待内容与截止时间，再结束轮次。
5. 唤醒后读日志、核对事件 ID，查看程序输出和退出码，再复查外部状态并决定下一步。验收通过才完成 Todo；程序退出本身不代表业务成功。
