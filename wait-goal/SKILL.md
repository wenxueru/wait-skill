...
- Waiting for external state: write a short single-line `event_note` saying what to do after wake-up, then pass it with `--event-note` when running `goal wait` with the program attached. It prepares the node, submits the watcher, and verifies the receipt in one command. Then run `activate-wait` with the returned watch ID and set up the client wake-up path.
